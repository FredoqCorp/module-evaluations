...
public sealed class ArithmeticMeanPolicyTests
{
    ...
    public void Arithmetic_mean_returns_expected_total()
    {
        ...
        var c1 = new FormCriterion(new Uuid(), new Criterion(new CriterionText("t1✓", "d1"), ImmutableList<IChoice>.Empty), new OrderIndex(0));
        var c2 = new FormCriterion(new Uuid(), new Criterion(new CriterionText("t2✓", "d2"), ImmutableList<IChoice>.Empty), new OrderIndex(1));
